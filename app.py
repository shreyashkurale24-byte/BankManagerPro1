...
All 36 tables | Login + 5-min idle timeout | Full CRUD
"""
...
if getattr(sys, 'frozen', False):
    ...
    SQL_FILE = os.path.join(BASE_DIR, "Dumptables.sql")
    ...
    DB_PATH = os.path.join(os.environ.get('APPDATA', os.path.expanduser('~')), "bank_manager_data.db")
else:
...
app.secret_key = "BankManagerPro@2024#Ultra"

# Determine if the application is running as a compiled standalone bundle
if getattr(sys, 'frozen', False):
    template_folder = os.path.join(sys._MEIPASS, 'bank_pro_v8', 'templates')
    # Use this logic to point to your Dumptables.sql path dynamically as well
else:
...
SQL_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Dumptables.sql")

# ── Demo credentials (change as needed) ──────────────────────────────────────
USERS = {"admin": "admin123", "manager": "bank2024"}
...
if __name__ == "__main__":
    ...
    print("  ✅  DB ready  |  http://127.0.0.1:5000")
